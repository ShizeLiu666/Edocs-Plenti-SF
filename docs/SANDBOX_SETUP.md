...
4. **公式注入防护(D-033)** —— 从你自己的外部邮箱往组里发一封**虚构**邮件,
   主题写 `=1+1`,正文第一行写 `+61 000 000 000`。跑一轮后看 `Messages` 页那一行:
   主题格**应显示 `=1+1` 原文**,正文格应显示 `+61 …` 原文。
   ❌ 如果主题格显示 `2`、或正文格变成数字/`#ERROR!`,**立刻关掉 `INTAKE_V2_ENABLED`**
   并告诉我 —— 说明单引号前缀在真实 Sheets 里不生效,离线测试测不到这一层。
5. **SF Lead ID 超链接(D-033)** —— 建出 Lead 的那一行,`SF Lead ID` 列应是可点的
   链接,点开进到该 Lead。旧行不会被回头改成链接,这是设计如此
