...
**⚠️ out-of-scope 改成 done —— 请 Jack 确认。** 依据是 Q18 的原文"review **只留给**
...
**如果 Jack 希望它也亮 Review,是 `plCreatedExceptions_` 里加一行。**
...
| ~~Q18~~ | ✅ **已关闭**(2026-09-11)—— 见 D-033。转介重发按 D-026 仍落 review;out-of-scope 改为 done **待 Jack 确认**。原文:**review 的语义要不要改成"脚本需要人帮忙",而不是"业务还没处理完"?** 即:干净建出的 referral 直接落 `done`,`review` 只留给降级、来源冲突、可疑发件人、错误。这样同时解决 D-031 的三处影响:合并会话里的 Review 平时不亮,**一旦亮就说明真有事**(包括同标题的冒充邮件);轮询只针对少数例外;"待补联系方式"整体交给 Salesforce List View | 无 | D-024 / D-029 / D-031 / D-033 |
| Q19 | **其他状态的长期累积。** D-032 只清理 out-of-scope(授权范围)。仍会累积:① internal / ignore / notice 等 `done` 状态,每条约 130 字节,量取决于同事往组里发多少邮件,**目前没有数据**;② 建出 Lead 的状态,每条约 400 字节,按约 5 条 referral/天约 2 KB/天,**约一年**写满。①的安全推理与 out-of-scope 完全相同,可以直接纳入清理;②不能简单删 —— 合并 referral 会话上的 Created 标签靠它们,要删就得先定标签语义(与 Q18、D-031 标签降级一起考虑)。都不阻塞上线 | 上线后数月内 | §9 / D-032 |
