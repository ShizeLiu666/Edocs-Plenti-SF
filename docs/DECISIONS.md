...
✅ **已在生产验证通过(2026-09-11,D-044)。** 原文:**⚠️ 离线测试只能证明单引号加上了,证明不了真实 Sheets 认它。** 我比较有把握它有效
...
## D-040 开触发器之前:公式注入实测、单轮耗时、重复 Lead、触发器配置

### ① 公式注入实测邮件的痕迹

从个人邮箱发主题 `=1+1`、正文第一行 `+61 000 000 000` 的邮件到组里。按代码它会走:
发件人不在可信清单 → 没有 Plenti 链接 → out-of-scope → `done`。

| 痕迹 | 处理 |
|---|---|
| `IV2_MSG_<id>`(最小形式约 75 字节) | **不用管。** 滑出扫描窗口后被 Q17 清理自动删掉 —— 顺带验证清理在生产上工作 |
| Messages 表一行 | 无害。代码从不读回这张表,手工删行不影响追加。留着可作验证记录 |
| Salesforce / Gmail 标签 | 都没有。不建 Lead,out-of-scope 不打标签 |

⚠️ 主题不能含 `test` 这个词(会被 `plExclude_` 判成"疑似测试邮件"走另一条路)。
⚠️ 如果组设置了主题前缀(如 `[edocs]`),主题就不以 `=` 开头,测不到转义。以表里
实际的主题格为准。
顺带:用 查看 → 显示公式 扫一遍已有的 88 行。D-033 之前(沙箱期)写入的行没有转义。

### ② 🔴 单轮耗时与扫描窗口 —— 开触发器前要量

**下一轮不是"只扫 watermark 之后"。** `lower = max(INTAKE_V2_START, watermark − 48h)`,
所以下一轮从 2026-09-09T08:16Z 扫起。48 小时重叠是有意的(防 Gmail 索引延迟)。
`INTAKE_V2_START` 现在只是下限,不再起作用。

**问题在于,处理过的邮件跳过得"便宜",但并不是零成本。** 窗口内的每一封邮件每一轮都会:
读收件人头(白名单检查在状态检查**之前**)、读两次 Script Properties;窗口内的每个会话
每一轮都会重新同步标签(一次 `getMessages` 加三个标签各一次查找和增删)。
**所以单轮成本跟 48 小时内的邮件量成正比,而不是跟新邮件量成正比。**

首次运行 56 个会话、74 封邮件,用了大约 4 分钟;主循环预算是 220 秒。满 48 小时的
窗口在工作日可能是这个量的两倍以上。**一旦单轮扫不完:`scanComplete:false` →
watermark 不前进 → 下一轮窗口更大 → 永远扫不完**,清理也跟着停(D-034 倒计时)。

Gmail 与 Properties 的每日配额也按同样的比例消耗:每 15 分钟一轮,一天 96 轮。
我没有可靠的配额数字,**不下结论**,但方向相同。

**量法:** 公式注入那次手动运行正好是一次稳态测量(几乎没有新邮件要处理,只是遍历
窗口)。看汇总页的 `Duration (s)` 列和 console 里的 `scanComplete`:
- 明显低于 60 秒:可以开;
- 接近或超过 90 秒:先别开。满窗口时很可能超出预算。

**可能的修法(未实施,需授权改 `runIntakeV2`):** 状态检查挪到白名单检查之前(已处理的
邮件不再读头);只对本轮有变化的会话同步标签(处理了邮件,或 review 被解除)。改完之后
单轮成本跟新邮件量成正比。

### ③ L-05 重新评估:按电话号码去重

Jack 提供:程序写入的是 Plenti 原样给的 10 位数字、无空格(形如 `0400000000`,此处为虚构号码)。

**先要核对的不只是"格式一不一致",还有 Salesforce 会不会替人改格式。** 据我所知,用户的
**区域设置(Locale)是英语(美国)时,手工输入的 10 位号码会被自动格式化成
`(040) 000-0000` 这种美国格式**。老板的时区已经确认设错了(④),区域设置很可能也是。所以 agent 核对
时要看**字段里实际存的值**,不是页面上看起来的样子。

**做法建议:** SOQL 用一组确定的格式变体做精确匹配(由规范化后的数字生成:
`0400000000`、`0400 000 000`、`+61400000000`、`+61 400 000 000`、`(040) 000-0000` 等,均为虚构),
查 `Phone` 和 `MobilePhone`,限定近 N 天创建。不用 SOSL:搜索索引有延迟,刚手工建的
Lead 可能还搜不到。
...
| 做法 | 好处 | 代价 |
|---|---|---|
| 不建,落 review | 不产生重复 | 这条 referral 的 token 和接收时间在 Salesforce 里没有记录 —— PLT001 对账和 token 去重都缺它;误报时真客户没有 Lead |
| 照建并分派,落 review | SLA 最安全 | 重复照样产生,Round Robin 照样给第二个销售 —— 就是今天的问题 |
| **照建但不写 `Lead_Category__c`,落 review `[POSSIBLE DUPLICATE of 00Q…]`**(建议) | Round Robin 不跑,**不会有第二个销售**;token、接收时间都在 Salesforce;误报时 Lead 停在 Lily 名下、有标签,她手动分派即可(和 `[NOT ROUTED]` 同一条路) | 仍有一条重复记录要人合并 |

误报风险:同住的人共用一个座机、老客户一年前的旧 Lead。所以要限定时间窗口。
这也部分解决了 Q15(跨邮箱去重)。

### ④ 重复 Lead 的清理:用合并,不要删除

**删掉程序建的那条,会把 `Plenti_Lead_ID__c` 和 `Plenti_Received_At__c` 一起删掉:**
- 同一条 referral 以后重发 → 找不到带这个 token 的 Lead → **再建一次**;
- 这条 referral 的 PLT001 接收时间从 Salesforce 消失,月度对账只能回头翻 Messages 表。

**Salesforce 的 Lead 合并(Merge)** 可以保留销售在跟进的那条作为主记录,并在逐字段对比里
选用程序那条的 `Plenti_Lead_ID__c` 和 `Plenti_Received_At__c`。已经删掉的,15 天内可从
回收站恢复后再合并。长文本字段在合并界面里能不能选,我不确定,以界面为准;这两个短字段
最要紧。
`IV2_MSG_*` / `IV2_CREATE_*` 仍然都保留(L-05)。

### ⑤ 触发器配置

1. **不需要加 `script.scriptapp`。** 代码(包括 Legacy)里没有任何 `ScriptApp` 调用。这个
   scope 是给"用代码建触发器"用的,而规格 §3 禁止写自动启用的辅助函数。触发器在编辑器的
   触发器页面手工建,不需要改 manifest,也就没有重新授权的问题。我比较确定;如果保存
   触发器时界面要求授权,照提示授权即可,那是界面自己的授权流程,不改代码。
2. **失败通知**:是在触发器的编辑对话框里,Failure notification settings → **Notify me
   immediately**。⚠️ 通知发给**建触发器的那个账号**。
3. **⚠️ 触发器必须用收件邮箱那个账号来建。** 触发器以创建者身份运行,`GmailApp` 读的是
   **创建者自己的邮箱**。用个人账号建的话,它会去搜你个人邮箱里的组邮件。失败通知也会发到
   那个收件邮箱 —— 要确认有人看。
4. 两条报警渠道要分清:**抛出的异常**(配置缺失、Gmail 出错、Properties 写满)→ 失败通知;
   **单封邮件落 error**(被拒、L-01)不会抛出 → 只在汇总页的 `Failures` 列可见。两边都要看。
5. 函数选 `runIntakeV2`,时间驱动,每 15 分钟。重叠的运行会被脚本锁挡掉,只打一行日志,不算失败。

### ⑥ 事实确认

老板的 Salesforce 用户时区是 GMT+5:30 —— D-039 ② 的推断成立,正在改。建议顺手看一下她的
**区域设置(Locale)**,它影响电话号码格式(③)和日期显示格式。Business Hours(Q8)仍开着,
agent 在查它被哪些地方用到。

---

## D-041 单轮成本改为跟新邮件量成正比 —— 开触发器的前置条件

Jack 2026-09-11 授权(D-040 ② 的两处修法都做)。首次生产运行 **221 秒,预算 220 秒**。

### 改动(`runIntakeV2` 主循环)

| | 之前 | 之后 |
|---|---|---|
| 窗口内**已处理**的邮件 | 每轮读收件人头(白名单)+ 读两次状态 | **只读一次状态**,不读头 |
| 标签同步 | 每轮同步窗口内**所有**会话(每个 7 次 Gmail 调用) | 只同步:本轮处理过邮件的、review 被解除的、**仍挂着 review 的** |
| 新邮件 / error 重试 | 白名单 → 处理 → 同步 | 不变 |

**为什么"仍挂着 review 的"也每轮同步 —— 这是对 Jack 所说"只同步有变化的"的一处补充:**
旧代码每轮同步所有会话,某一轮同步失败(Gmail 瞬时错误),下一轮自然补上。只同步有变化的
会话会失去这种自愈;而 Q18 之后,Review 是唯一还要紧的标签。review 只留给例外,量很少,
成本可以忽略。Created 标签没有这层自愈,D-031 之后它已降级为装饰。

**跳过白名单检查为什么安全:** 状态只会为命中白名单的邮件写入(主循环与 R9 入口都先查
白名单)。唯一的行为差别:把某个地址移出白名单之后,它名下已处理、仍在 review 的邮件还会
继续被刷新。可以忽略。

稳态单轮成本:每个会话一次 `getMessages`,每封邮件一次状态读取,外加例外会话的标签同步。
**实际耗时要看部署后下一次手动运行的 `Duration (s)`**(D-040 ② 的阈值不变)。

### 验证

离线测试第 11 节:第一轮覆盖五种会话(已完成、仍在 review、新邮件、error 重试、review
被解除);第二轮(没有任何变化)断言**零次读头、零次 SOQL、只同步仍在 review 的会话**。
四个变异(恢复全量同步、不同步 review 会话、不同步被解除的会话、先读头再查状态)全部
被对应的断言捕获。

---

## D-042 PLT001 的"1 个工作日"怎么算(分析,待定);Round Robin 被改过

### ① Business Hours 查清楚了:完全孤立

org 只有一条 `Default`:America/Los_Angeles、24/7、零假期。没有任何东西引用它
(Entitlement 未启用、Case 升级 / 自动回复规则为空、两个 Flow 不涉及、Lead 上没有相关公式)。
24/7 + 零假期等于"永远在营业时间内",所以 LA 时区**暂时**不产生影响。

**建议不论下面怎么定,都把它的时区改成 Australia/Adelaide。** 零风险,而且 agent 指出的坑是
真的:一旦有人把它改成具体时段,LA 时区会让营业时间落在阿德莱德的半夜。

### ② PLT001 放在哪里算

**先说一个会影响选择的事实:Salesforce 的普通公式字段和报表都不能按 Business Hours 算时长。**
按营业时间计算只在这几处可用:Entitlement / Milestone(**只支持 Case**,而且未启用)、
Apex 的 `BusinessHours` 类、Case 升级规则。Lead 上的公式最多只能用星期几的算术近似
"排除周末",**节假日做不到**。所以"配好 Business Hours + Holidays,用报表算"在 Lead 上
**不能直接成立** —— 除非有人写 Apex,而本项目不往 Salesforce 部署任何东西。

| 做法 | 怎么做 | 代价 |
|---|---|---|
| A. Salesforce 内算 | Business Hours + Holidays + Apex 触发器或定时任务 | 要 Sunterra 管理员写和维护 Apex |
| **B. 代码在建 Lead 时算截止时间**(建议) | 用 `Plenti_Received_At__c` 按阿德莱德时区、南澳公共假期算出"最迟联系时间",写进一个新的 DateTime 字段。Salesforce 只做比较:List View 筛"截止时间已过、还没联系" | 假期表要每年维护(放进 Script Property);要建一个新字段 |
| C. 事后对账 | 月底从导出数据在表格里算 | 只能事后知道违约,**防不住** |

**建议 B,理由:**
- 最难的部分(工作日与假期的算术)只在一处做,可以离线写测试;
- 时区在代码里是显式的,不依赖任何人的 Salesforce 个人设置(今天已经证明这些设置不可靠);
- 产出的是一个**预警**(截止时间),不只是事后统计 —— PLT001 违约 Plenti 可以立即终止合同,
  防住比统计到更要紧。

选 B 的话,那条 Business Hours 可以先不动(但仍建议改时区)。

**不管选哪种,都要先定两件业务上的事,代码替代不了:**
1. **合同里"1 个工作日"的确切定义** —— 周五下午 4 点收到,截止是周一下午 4 点,还是周一
   下班?按哪个州的公共假期?营业时间是几点到几点?
2. **什么事件算"已尝试联系"。** 目前 Salesforce 里没有可靠的"首次联系时间":
   `Contact_Attempt_Count__c` 被 Governance Flow 在创建时覆盖成 0,之后由谁、怎么累加不清楚。
   是看记下的通话 Task、Status 变化,还是这个计数?**没有终点就测不出是否违约**,
   只能做到截止预警。

Q8 保持开放,附上本条。

### ③ Round Robin 今天 4:54 PM 被人改过(不是 agent 改的)

**这个时间是哪个时区显示的,决定了我们的验证还算不算数:**
- 如果是阿德莱德时间 → 07:24Z,在首次运行(08:16Z 开始)**之前**。三条 Lead 被正确分派,
  验证的就是现在这个版本。
- 如果是 GMT+5:30(老板账号的时区)→ 11:24Z = 阿德莱德 20:54,在首次运行**之后**。
  那么**现在生效的版本没被验证过**。

**建议开触发器前,让 agent 再读一次它的进入条件,和 D-038 记下的五条逐一对比。**

这也是 D-038 里那个风险第一次真实出现:Flow 可以在不通知任何人的情况下被改。代码能自己
发现分派失败:建完回读 Lead 时,After Save 已经跑完,**`OwnerId` 仍等于 `CreatedById`
就说明 Round Robin 没接住**,落 review `[NOT ROUTED]`。改动很小(回读字段加一个
`CreatedById`,它在任何 org 都存在)。**未实施**,建议排在触发器跑稳之后、和电话去重一起做。

---

## D-043 电话写两个字段的副作用;PLT001 拿不到终点时怎么办;合并暂停

### ① 电话:格式一致,问题是字段不同(Jack,2026-09-11)

agent 核对:手工建的 Lead 存在 `Phone`,程序建的存在 `MobilePhone`;号码字符串完全相同
(本地格式、无空格、无国家码)。老板的 Locale 是 English (Australia),没有美式格式化。
**所以电话去重在格式上没有障碍** —— D-040 ③ 的顾虑解除。

**新问题:** 程序建的 Lead 只有 `MobilePhone`。列表视图或拨号工具如果只读 `Phone`,这些
Lead 看起来就没有电话。Jack 建议两个字段都写。

**我的判断:可以写,但有三个副作用必须一起处理,另有一项要先查:**

| # | 副作用 | 处理 |
|---|---|---|
| 1 | **review 被错误解除。** `plSuppliedContacts_` 只记一个字段的指纹。只改写入的话,我们自己写进 `Phone` 的号码下一轮会被 `plRefreshReview_` 当成"人补的",例外 review(包括 `[NOT ROUTED]` 报警)被自动解除 | 指纹两个字段都记。必须和写入同一次改 |
| 2 | **座机不能进 `MobilePhone`。** 短信工具会去给座机发短信 | 只有手机号(04 开头)两边都写;座机照旧只写 `Phone` |
| 3 | **两处副本会分叉。** 销售改了其中一个号码,另一个还是旧的 | 接受。转换时两个字段各自映射到 Contact,不丢数据 |
| ⚠️ | **Duplicate Rule。** agent 查过 Validation Rule,**没查 Duplicate Rule**。如果有 Active 的 Lead 重复规则按 `Phone` 匹配,且动作是 Block,写了 `Phone` 之后程序建的 Lead 会撞上手工建的那条被拒(`DUPLICATES_DETECTED`)→ 落 error、每轮重试、watermark 冻结 | **改之前先查 Setup → Duplicate Rules(Lead)**:有没有 Active 的、按什么匹配、动作是 Alert 还是 Block。只是 Alert 的话 API 创建不受影响 |

改动很小(`plLeadPayload_` 与 `plSuppliedContacts_`),但它改的是 D-029 的"手机进
`MobilePhone`、其余进 `Phone`"。**等 Jack 定时机**:可以现在做,也可以和电话去重一起做
(两者都动电话逻辑)。

### ② PLT001 拿不到可靠终点时,方案退化成什么

测 PLT001 要一个"首次尝试联系"的时刻。按可靠程度从高到低:

| 级别 | 终点从哪来 | 需要谁做什么 | 能给出什么 |
|---|---|---|---|
| 1 | **电话系统自动记录通话**(拨号工具把每通电话自动记成 Task) | 问:销售是用集成了 Salesforce 的电话系统打,还是用自己的手机? | 精确的首次联系时刻 |
| 2 | **字段历史跟踪**:给 Lead 的 `Status`(和 / 或 `Contact_Attempt_Count__c`)开 Field History Tracking | Salesforce 管理员勾选,不写代码。约定:**尝试联系时把 Status 从 New 改掉** | Lead History 报表里有"第一次改 Status"的时刻 |
| 3 | **截止时刻快照**:脚本本来每 15 分钟跑一轮。某条 Lead 的截止时间一过,就读一次 `Status`、`Contact_Attempt_Count__c`、`LastActivityDate`,有任何联系痕迹记"达标",否则记"违约",写进 Salesforce 字段或表格 | 代码(以后做)。依赖销售在截止前**留下任何一种痕迹** | 每条 Lead 达标 / 违约的二元结果。季度 PLT001 = 达标数 ÷ 总数 |
| 4 | 什么痕迹都没有 | —— | **只能预警,测不出**。季度报告里的 PLT001 没有依据,必须如实告诉老板和 Plenti |

**第 3 级的关键洞察:SLA 问的是"截止之前有没有联系",是一个是 / 否的问题,并不需要知道
几点几分联系的。** 所以即使永远拿不到首次联系时刻,只要销售在截止前留下任何痕迹,
在截止那一刻看一眼就能判定。它比第 2 级粗,但**只依赖一个很低的流程要求**。

**所有级别共同的底线:** 销售用自己的手机打、Salesforce 里什么都不记 —— 这种情况任何工具
都测不出来。**最低的流程要求是"尝试联系时把 Status 从 New 改掉"**,操作量最小,同时满足
第 2、3 级。

**比季度报告更要先问的一件事:Plenti 自己是怎么判定 PLT001 的?** 如果 Plenti 看的是它
Portal 里合作方更新的转介状态(我们联系客户的方式本来就要登录 Portal),那**权威数据在
Plenti 那边**,季度数字以它的报告为准,我们要做的只是别违约。这个问题的答案可能让上面
整张表变成"只要预警就够"。

### Jack 的决定(2026-09-11)

- **电话写两个字段 + 电话去重:一起做,排在触发器跑稳之后。** 两件都动电话逻辑,一次改完
  更安全。改之前 agent 先查 Lead 的 Duplicate Rule。只有 04 开头的手机号两边都写,座机
  只写 `Phone`。
- **SLA 相关的代码冻结,直到问清"Plenti 自己怎么判定 PLT001"。** 如果权威数据在 Plenti
  的 Portal 里,四级全部简化为"只做预警"。
- **流程要求"尝试联系时把 Status 从 New 改掉"由 Jack 推动。** 已有一个佐证:首批三条里,
  一条今天已被跟进的销售改成 Working —— 至少有人已经这么做了。

### ③ 合并暂停(Jack)

三对重复都有销售在跟进,Round Robin 分到的三个人会因合并作废,其中一人已经动过其中一条。
**这是人的决定**,Jack 去问老板。

对管道来说**哪条留下都可以**,只要留下的那条带着 `Plenti_Lead_ID__c` 和
`Plenti_Received_At__c`:
- 留程序建的那条:什么都不用复制;
- 留手工建的那条:合并时这两个字段选程序那条的值。

没定之前两条都在,`IV2_MSG_*` / `IV2_CREATE_*` 照旧保留,不影响任何运行。

---

## D-044 开触发器前的三项技术验收全部通过

Jack 2026-09-11 在生产完成(部署到 `156f036`)。

| 项 | 结果 |
|---|---|
| **公式注入**(D-033,SANDBOX_SETUP §9 第 4 步) | ✅ 三重确认:公式栏显示 `'=1+1`(前导撇号);单元格显示 `=1+1` 且左对齐(文本);gviz 导出拿到的是字符串而不是数字。agent 全量扫描 97 行 × 12 列 = 1164 个单元格,**零公式、零错误值**,沙箱期的历史行也没有遗留 |
| **单轮耗时**(D-041) | ✅ **32 秒**(优化前 221 秒,预算 220 秒)。`reviewed` 从 74 降到 9 —— 已处理的邮件不再重新读取 |
| **Q17 容量**(D-032) | ✅ 前两轮已验证 |

D-033 那句"离线测试证明不了真实 Sheets 认它"就此关闭。

### 残留风险:导出 CSV(记录,不改)

前导撇号只存在于 Sheets 内部,**导出 CSV 时会被剥掉**。有人把表导成 CSV 再用 Excel 打开,
以 `=` 开头的单元格会被重新执行。

Jack 的判断:**优先级不高,不改。** 要同时满足"导出"和"用 Excel 打开"两个条件;这张表的
用途是排查不是分发;改成可见的撇号或别的前缀会让表难读。已写进 README 已知限制:
**不要导出这张表给外部使用。**

补一点:据我所知,导出为 .xlsx 时文本单元格仍按字符串保存,Excel 不会执行。所以真要导出,
用 .xlsx 而不是 CSV。这一点我没有实测。

### 开触发器还差的两件事(都不是技术问题)

1. 老板停止手工建 Plenti 的 Lead(L-05);
2. 核对 Round Robin 今天下午被改了什么(D-042 ③),与 D-038 的进入条件逐条对比。

---

## D-045 Round Robin 的改动核对完毕;Duplicate Rule 不是阻碍

### ① Round Robin V4 → V5:进入条件一个字符没变

agent 逐字符比对完整表达式(318 个字符),与 D-038 记录的**完全一致**。改的是新增的
休假绕行逻辑:某位销售休假期间(到 2026-09-18 07:20 UTC 为止),原本分给轮值位置 2 的
改派给位置 3。六个 User ID 都没动,只是驱动位置的那个数换了来源。
**对我们的链路零影响。**(休假期间位置 3 会多接一份,属于业务侧安排。)

**D-038 的风险第一次真实发生:** Flow 在没通知任何人的情况下被改了。这次无害,但如果哪天
改的是进入条件,**我们现在没有任何办法自动发现**。回读 `OwnerId` 与 `CreatedById`
比对的检测(D-042 ③)价值因此得到确认,仍排在触发器跑稳之后。

### ② Duplicate Rule:两条启用中的规则,动作都是 Allow

D-043 ① 要求的检查已完成。Lead 上两条启用中的 Duplicate Rule,Action On Create 都是
**Allow**。只有 Block 才会让 API 插入抛出 `DUPLICATES_DETECTED`,所以**拦不住程序建 Lead**。
电话写两个字段没有阻碍,仍排在触发器跑稳之后、与电话去重一起做。

**推理上的一处精确化:** "今天三条与手工建的姓名、电话完全一样,照样 0 failures"这条证据
**单独不足以判定** —— 今天两边的号码在不同字段(`Phone` 与 `MobilePhone`),就算匹配规则
按 `Phone` 比对也匹配不上。**起决定作用的是 Allow 这个动作。** 标准匹配规则包含哪些字段
查不到(Salesforce 托管),但既然是 Allow,就不影响结论。

**做电话去重时顺手看一眼:** 这两条规则有没有勾选 **Report**。据我所知,Allow + Report 时,
Salesforce 会为检测到的重复生成 Duplicate Record Set 记录。如果是这样,就多了一个现成的
重复信号,可以和我们自己的电话匹配互相印证。我没有实测,以实际配置为准。

### 开触发器还剩一件

老板停止手工建 Plenti 的 Lead(L-05),Jack 正在确认。
...
**处置(2026-09-11):** 原计划删掉程序建的那两条。⚠️ **D-040 ④ 建议改为合并**:
删除会连同 `Plenti_Lead_ID__c` / `Plenti_Received_At__c` 一起丢掉,以后同一 referral
重发会再建一次,PLT001 对账也少了这一条。
...
- 技术上(**D-040 ③ 已重新评估,待 Jack 定命中后的做法**):建之前按电话号码找近期 Lead。
...
| Q8 | ⏳ **2026-09-11 已查清(D-042)**:唯一一条 Business Hours 完全孤立,改时区零风险;Lead 上的公式和报表不能按营业时间算,建议由代码在建 Lead 时算出截止时间。**先要定合同里"1 个工作日"的定义和"已尝试联系"的判据。** 原文:Business Hours 修正(当前是 Los Angeles + 24/7,须改 Adelaide + 南澳公共假期)。本项目之外的 Salesforce 配置任务,但在修好前任何"工作日"计算都是错的 | SLA 计算 | §4 |
